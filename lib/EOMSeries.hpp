...
#ifndef _SERIES_EOMSERIES_HPP_
#define _SERIES_EOMSERIES_HPP_
...
/*!
  EOMSeries is an STL map collection indexed by date.
*/
class EOMSeries: public std::map<boost::gregorian::date, Series::MonthPrice>
{
  typedef std::map<boost::gregorian::date, MonthPrice> ThisMap;
...
public:
  ...
  */
  EOMSeries(const std::string& name);
  ...
  */    
  ...
  //! Return last EOM record in series.
  Series::MonthPrice last(void) const { return (*rbegin()).second; }
